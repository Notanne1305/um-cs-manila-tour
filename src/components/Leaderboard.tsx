            ...
            <tbody>
              {donors.map((donor, index) => {
                ...
                const getRankStyles = () => {
                  if (donor.rank === 1) {
                    return {
                      container: "w-14 h-14 rounded-xl bg-gradient-to-br from-yellow-400 via-yellow-500 to-amber-600 shadow-lg shadow-yellow-500/30",
                  ...
                  if (donor.rank === 2) {
                    return {
                      container: "w-12 h-12 rounded-xl bg-gradient-to-br from-slate-300 via-slate-400 to-slate-500 shadow-lg shadow-slate-400/30",
                  ...
                  if (donor.rank === 3) {
                    return {
                      container: "w-11 h-11 rounded-xl bg-gradient-to-br from-amber-600 via-orange-600 to-orange-700 shadow-lg shadow-orange-500/30",
                    ...
                    <td className={`py-4 px-4 md:px-6 ${isTopThree ? 'py-5' : ''}`}>
                      <div className="flex items-center justify-center">
                        {icon ? (
                          ...
                          <div className={`flex items-center justify-center text-sm font-semibold text-muted-foreground ${styles.container}`}>
                            {donor.rank}
                          </div>
                        )}
                      </div>
                    </td>
                    <td className={`py-4 px-4 md:px-6 font-medium ${isTopThree ? 'text-lg font-semibold text-foreground' : 'text-foreground'}`}>
                      {donor.name}
                    </td>
                    <td className="py-4 px-4 md:px-6 text-right">
                      <span className={`font-semibold ${isTopThree ? 'text-lg text-primary' : 'text-primary'}`}>
